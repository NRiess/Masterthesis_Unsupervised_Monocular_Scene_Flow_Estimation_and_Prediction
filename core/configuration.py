...
class ModelAndLoss(nn.Module):
    def __init__(self, args, model, training_loss, evaluation_loss=None, test_loss=None):
    ...
    # -------------------------------------------------------------
    def forward(self, example_dict, epoch, args):
        # -------------------------------------
        # Run forward pass
        # -------------------------------------
        ...
        # -------------------------------------
        if self.training:
            loss_dict, loss_dict_tensorboard, img_l2_warp, occ_map_f = self._training_loss(output_dict, example_dict, epoch, args)
        elif self.test:
            loss_dict, loss_dict_tensorboard, occ_map_f = self._test_loss(output_dict, example_dict)
        elif self.validation:
            try:
                loss_dict, loss_dict_tensorboard = self._evaluation_loss(output_dict, example_dict, epoch)
            except:
                loss_dict, loss_dict_tensorboard, img_l2_warp, occ_map_f = self._evaluation_loss(output_dict, example_dict, epoch, args)
        ...
        # -------------------------------------
        if self.training:
            return loss_dict, loss_dict_tensorboard, output_dict, img_l2_warp, occ_map_f
        else:
            if self.test:
                return loss_dict, loss_dict_tensorboard, output_dict, occ_map_f
            else:
                return loss_dict, loss_dict_tensorboard, output_dict, img_l2_warp, occ_map_f
...
def configure_runtime_augmentations(args):
    with logger.LoggingBlock("Runtime Augmentations", emph=True):
        ...
        if args.test_augmentation is not None:
            kwargs = tools.kwargs_from_args(args, "test_augmentation")
            logging.info("test_augmentation: %s" % args.test_augmentation)
            for param, default in sorted(kwargs.items()):
                logging.info("  %s: %s" % (param, default))
            kwargs["args"] = args
            test_augmentation = tools.instance_from_kwargs(
                args.test_augmentation_class, kwargs)
            if args.cuda:
                test_augmentation = test_augmentation.cuda()

        else:
            logging.info("test_augmentation: None")

    return training_augmentation, validation_augmentation, test_augmentation
...
def configure_model_and_loss(args):
    ...
    with logger.LoggingBlock("Model and Loss", emph=True):
        ...
        if args.test_loss is not None:
            kwargs = tools.kwargs_from_args(args, "test_loss")
            kwargs["args"] = args
            test_loss = tools.instance_from_kwargs(args.test_loss_class, kwargs)

        # ----------------------------------------------------
        # Model and loss
        # ----------------------------------------------------
        model_and_loss = ModelAndLoss(args, model, training_loss, validation_loss, test_loss)
...
class CheckpointSaver:
    ...
    def save_latest(self, directory, model_and_loss, stats_dict, dt_string, store_as_best=False):
        # -----------------------------------------------------------------------------------------
        ...
        # -----------------------------------------------------------------------------------------
        if store_as_best:
            best_checkpoint_filename = os.path.join(directory, dt_string + self._prefix + self._best_postfix + self._extension)
...
def configure_data_loaders(args):
    with logger.LoggingBlock("Datasets", emph=True):
        ...
        if args.test_dataset is not None:
            ...
            # ----------------------------------------------
            # Figure out test_dataset arguments
            # ----------------------------------------------
            kwargs = tools.kwargs_from_args(args, "test_dataset")
            kwargs["is_cropped"] = True
            kwargs["args"] = args
            ...
            # Create test dataset
            # ----------------------------------------------
            test_dataset = tools.instance_from_kwargs(args.test_dataset_class, kwargs)
            ...
            _log_statistics(validation_dataset, prefix="Test", name=args.validation_dataset)
