...
def update_config(config, args, im_size, channel_size, embed_dim, num_heads, window_size):
    ...
    if args.patch_size:
        config.MODEL.SWIN.PATCH_SIZE = 1
    if args.window_size:
...
def get_config(args, im_size, channel_size, embed_dim, num_heads,window_size):
    """Get a yacs CfgNode object with default values."""
    # Return a clone so that the defaults will not be altered
    # This is for the "local variable" use pattern
    config = _C.clone()
    update_config(config, args, im_size, channel_size, embed_dim, num_heads,window_size)
