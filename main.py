...
def main():
    ...
    # DataLoader
    train_loader, validation_loader, test_loader = config.configure_data_loaders(args)
    success = any(loader is not None for loader in [train_loader, validation_loader, test_loader])
    if not success:
        logging.info("No dataset could be loaded successfully. Please check dataset paths!")
        quit()

    # Configure data augmentation
    training_augmentation, validation_augmentation, test_augmentation = config.configure_runtime_augmentations(args)
