...
class KITTI_Raw(data.Dataset):
    def __init__(self,
        ...
        filenames = filenames[:int(len(filenames) / 1)]
        ...
        for item in filename_list:
            ...
            # if os.path.isfile(name_l1) and os.path.isfile(name_l2) and os.path.isfile(name_r1) and os.path.isfile(
            #         name_r2):
            ...
            for i in range(0, self._args.num_of_past_frames):
                name_l.append(os.path.join(images_root, date, scene, view1, '%.10d' % (int(idx_src) + i)) + ext)
                name_r.append(os.path.join(images_root, date, scene, view2, '%.10d' % (int(idx_src) + i)) + ext)
            name_lr=name_l+name_r
            path_exists=True
            for i in range(0, self._args.num_of_past_frames):
                if not os.path.isfile(name_l[i]) or not os.path.isfile(name_r[i]):
                    path_exists = False
            if path_exists:
                self._image_list.append(name_lr)
    ...
    def __getitem__(self, index):
        ...
        basename = os.path.basename(im_l0_filename)[:6]
        dirname = os.path.dirname(im_l0_filename)[-51:]
        datename = dirname[:10]
        k_l0 = torch.from_numpy(self.intrinsic_dict_l[datename]).float()
        k_r0 = torch.from_numpy(self.intrinsic_dict_r[datename]).float()
        # input size
        h_orig, w_orig, _ = img_list_np[0].shape
        input_im_size = torch.from_numpy(np.array([h_orig, w_orig])).float()
        # cropping
        if self._preprocessing_crop:
            ...
            # cropping images and adjust intrinsic accordingly
            img_list_np = kitti_crop_image_list(img_list_np, crop_info)
            k_l0, k_r0 = kitti_adjust_intrinsic(k_l0, k_r0, crop_info)
        # to tensors
        img_list_tensor = [self._to_tensor(img) for img in img_list_np]
        im_l = []
        im_r = []
        for i in range(0, self._args.num_of_past_frames):
            im_l.append(img_list_tensor[i])
        for i in range(self._args.num_of_past_frames, 2*self._args.num_of_past_frames):
            im_r.append(img_list_tensor[i])
        common_dict = {
            "index": index,
            "basename": basename,
            "datename": datename,
        ...
        if self._flip_augmentations is True and torch.rand(1) > 0.5:
            ...
            im_r_flip = []
            for i in range(0, self._args.num_of_past_frames):
                im_l_flip.append(torch.flip(im_l[i], dims=[2]))
                im_r_flip.append(torch.flip(im_r[i], dims=[2]))
            k_l0[0, 2] = ww - k_l0[0, 2]
            k_r0[0, 2] = ww - k_r0[0, 2]
            for i in range(0, self._args.num_of_past_frames):
                example_dict["input_l"+str(i+1)]= im_r_flip[i]
                example_dict["input_r" + str(i+1)]= im_l_flip[i]
                example_dict["input_k_l"+str(i+1)]= k_r0
                example_dict["input_k_r" + str(i+1)]= k_l0
            example_dict.update(common_dict)
        else:
            for i in range(0, self._args.num_of_past_frames):
                example_dict["input_l"+str(i+1)]= im_l[i]
                example_dict["input_r" + str(i+1)]= im_r[i]
                example_dict["input_k_l"+str(i+1)]= k_l0
                example_dict["input_k_r" + str(i+1)]= k_r0
            example_dict.update(common_dict)
        return example_dict
