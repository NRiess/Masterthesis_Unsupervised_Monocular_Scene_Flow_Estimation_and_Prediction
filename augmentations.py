...
class PhotometricAugmentation(nn.Module):
    ...
    def forward(self, *args):
        ...
        rand_gamma = torch.rand([b, 1, 1, 1], dtype=d_dtype, device=d_device,
                                requires_grad=False) * self._intv_gamma + self._min_gamma
        rand_brightness = torch.rand([b, 1, 1, 1], dtype=d_dtype, device=d_device,
                                     requires_grad=False) * self._intv_brght + self._min_brght
        rand_shift = torch.rand([b, 3, 1, 1], dtype=d_dtype, device=d_device,
                                requires_grad=False) * self._intv_shift + self._min_shift
...
class Augmentation_ScaleCrop(nn.Module):
    def __init__(self, args, photometric=True, trans=0.07, scale=[0.93, 1.0], resize=[128, 416]): #resize=[256, 832]
    ...
    def find_invalid(self, img_size, params):
        ...
        pt_mat = torch.cat([torch.cat([hf_w, hf_o, hf_o], dim=2), torch.cat([hf_o, hf_h, hf_o], dim=2),
                            torch.cat([hf_o, hf_o, hf_i], dim=2)], dim=1)
        ...
        invalid = (((x_tf <= -img_hf_w) | (y_tf <= -img_hf_h) | (x_tf >= img_hf_w) | (y_tf >= img_hf_h)).sum(dim=1,
    ...
    def calculate_tform_and_grids(self, img_size, resize, params):
        ...
        grid_pts = torch.cat([grid_ww, grid_hh, torch.ones_like(grid_hh)], dim=0).unsqueeze(0).expand(self._batch, -1,
                                                                                                      -1, -1)
    ...
    def find_aug_params(self, img_size, resize):
        ...
        while invalid.sum() > 0:
            ...
            max_t = torch.min(torch.ones_like(scale) - scale,
                              max_trans) * 0.5  # 0.5 because the translation rage is [-0.5*trans, 0.5*trans]
...
class Augmentation_SceneFlow(Augmentation_ScaleCrop): # training
    def __init__(self, args, photometric=True, trans=0.07, scale=[0.93, 1.0], resize=[128, 416]):  # resize=[256, 832]
    ...
    def forward(self, example_dict):
        ...
        for i in range(0, self._args.num_of_past_frames):
            im_l.append(example_dict["input_l" + str(i + 1)])
            im_r.append(example_dict["input_r" + str(i + 1)])
            k_l.append(example_dict["input_k_l" + str(i + 1)])
            k_r.append(example_dict["input_k_r" + str(i + 1)])
        self._batch, _, h_orig, w_orig = im_l[0].size()
        self._device = im_l[0].device
        ## Finding out augmentation parameters
        params = self.find_aug_params([h_orig, w_orig], self._resize)
        coords = self.calculate_tform_and_grids([h_orig, w_orig], self._resize, params)
        params_scale, _, _, _ = self.decompose_params(params)
        ## Augment images
        for i in range(0, self._args.num_of_past_frames):
            im_l[i] = tf.grid_sample(im_l[i], coords, align_corners=True)
            im_r[i] = tf.grid_sample(im_r[i], coords, align_corners=True)
        ## Augment intrinsic matrix
        k_list=[]
        for i in range(0, self._args.num_of_past_frames):
            k_list.append(k_l[i].unsqueeze(1))
        for i in range(0, self._args.num_of_past_frames):
            k_list.append(k_r[i].unsqueeze(1))
        num_splits = len(k_list)
        intrinsics = torch.cat(k_list, dim=1)
        intrinsics = self.augment_intrinsic_matrices(intrinsics, num_splits, [h_orig, w_orig], self._resize, params)
        k_lr = list(torch.chunk(intrinsics, num_splits, dim=1))
        k_l = k_lr[:self._args.num_of_past_frames]
        k_r = k_lr[self._args.num_of_past_frames:]
        for i in range(0, self._args.num_of_past_frames):
            k_l[i] = k_l[i].squeeze(1)
            k_r[i] = k_r[i].squeeze(1)
        ## Photometric augmentation
        if self._photometric and torch.rand(1) > 0.5:
            im_lr = self._photo_augmentation(im_l, im_r)
            im_l = im_lr[:self._args.num_of_past_frames]
            im_r = im_lr[self._args.num_of_past_frames:]
        ## construct updated dictionaries
        example_dict["input_coords"] = coords
        example_dict["input_aug_scale"] = params_scale
        k_l_flip = []
        k_r_flip = []
        for i in range(0, self._args.num_of_past_frames):
            example_dict["input_l"+str(i+1)+'_aug'] = im_l[i]
            example_dict["input_r" + str(i + 1) + '_aug'] = im_r[i]
            example_dict["input_k_l" + str(i + 1) + '_aug'] = k_l[i]
            example_dict["input_k_r" + str(i + 1) + '_aug'] = k_r[i]
            k_l_flip.append(k_l[i].clone())
            k_r_flip.append(k_r[i].clone())
            k_l_flip[i][:, 0, 2] = im_l[i].size(3) - k_l_flip[i][:, 0, 2]
            k_r_flip[i][:, 0, 2] = im_r[i].size(3) - k_r_flip[i][:, 0, 2]
            example_dict["input_k_l"+str(i+1)+"_flip_aug"] = k_l_flip[i]
            example_dict["input_k_r" + str(i + 1) + "_flip_aug"] = k_r_flip[i]
        aug_size = torch.zeros_like(example_dict["input_size"])
        aug_size[:, 0] = self._resize[0]
        aug_size[:, 1] = self._resize[1]
        example_dict["aug_size"] = aug_size
...
class Augmentation_Resize_Only(nn.Module):          # test, validation
    def __init__(self, args, photometric=False, imgsize=[128, 416]):  # imgsize=[256, 832]
    ...
    def forward(self, example_dict):
        if self._args.test:
            ...
            self._isRight = True
            for i in range(0, self._args.num_of_past_frames):
                if 'input_r'+str(i+1) not in example_dict:
                    self._isRight=False
            # Focal length rescaling
            _, _, hh, ww = example_dict["input_l1"].size()
            sy = self._imgsize[0] / hh
            ...
            im_l, k_l = [],[]
            for i in range(0, self._args.num_of_past_frames):
                im_l.append(interpolate2d((example_dict["input_l"+str(i+1)]), self._imgsize))
                k_l.append(_intrinsic_scale(example_dict["input_k_l"+str(i+1)], sx, sy))
            if self._isRight:
                im_r, k_r = [], []
                for i in range(0, self._args.num_of_past_frames):
                    im_r.append(interpolate2d((example_dict["input_r" + str(i + 1)]), self._imgsize))
                    k_r.append(_intrinsic_scale(example_dict["input_k_r" + str(i + 1)], sx, sy))
            if self._photometric and torch.rand(1) > 0.5:
                if self._isRight:
                    im_lr = self._photo_augmentation(im_l, im_r)
                    im_l = im_lr[:self._args.num_of_past_frames]
                    ...
                    im_l = self._photo_augmentation(im_l)
            for i in range(0, self._args.num_of_past_frames):
                example_dict["input_l"+str(i+1)+"_aug"] = im_l[i]
                example_dict["input_k_l" + str(i + 1) + "_aug"] = k_l[i]
            if self._isRight:
