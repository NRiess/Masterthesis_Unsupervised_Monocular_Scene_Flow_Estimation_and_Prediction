...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, x_warped=None, mask=None):
        """
        Args:
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        if self.shift_size[0] > 0 or self.shift_size[1] > 0:
            ...
            h_slices = (slice(0, (-1) * self.window_size[0]),
                        slice((-1) * self.window_size[0], (-1) * self.shift_size[0]),
                        slice((-1) * self.shift_size[0], None))
            w_slices = (slice(0, (-1) * self.window_size[1]),
                        slice((-1) * self.window_size[1], (-1) * self.shift_size[1]),
                        slice((-1) * self.shift_size[1], None))
            cnt = 0
            for h in h_slices:
                for w in w_slices:
                    img_mask[:, h, w, :] = cnt
    ...
    def forward(self, x, x_warped=None):
        H, W = self.input_resolution
        B, L, C = x.shape
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 use_checkpoint=False, args=None, **kwargs):
        super().__init__()
        ...
        if self.ape:
            ...
            if args.version=="stacked":
                ...
                self.num_inputs=3
            self.absolute_pos_embed = nn.Parameter(torch.zeros(1, int(in_chans/self.num_inputs), img_size[0], img_size[1]))
    ...
    def forward_features(self, x, l, args, x_warped=None):  # 2x1024x4x16
    ...
    def forward(self, x, l, args, x_warped=None):
        x = self.forward_features(x, l, args, x_warped)
        return x
