...
class ContextNetwork(nn.Module):
    def __init__(self, ch_in, args):
        ...
        if args.version=="predict":
            self.conv_sf_pred = conv(32, 3, isReLU=False)
            self.conv_d1_pred = nn.Sequential(conv(32, 1, isReLU=False), torch.nn.Sigmoid())
...
class ContextNetworkPred(nn.Module):
    def __init__(self, ch_in):
        super(ContextNetworkPred, self).__init__()
    ...
    def forward(self, x):
        x_out = self.convs(x)
        sf_pred = self.conv_sf_pred(x_out)
        disp1_pred = self.conv_d1_pred(x_out) * 0.3

        return sf_pred, disp1_pred
